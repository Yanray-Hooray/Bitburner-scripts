...
export async function main(ns) {
  async function workAssessment(target, threadsAvail) {
    ...
    if (test1 != 0 && test2 != Infinity && test3 != Infinity) {
      ...
      var nthreads2w2 = Math.ceil(ns.growthAnalyzeSecurity(nthreads2g) / 0.05);
      ...
      var hpayout = 0.9 * ns.getServerMaxMoney(target);
      var nthreads2h = Math.ceil(0.9 / ns.hackAnalyze(target));
      ...
      threadsMax = threadsMax.sort(function (a, b) {
        return b - a;
      });
      ...
      if (nthreads2w2 > threadsAvail) {
        time2w2 = Math.ceil(
          ns.getWeakenTime(target) + threadsAvail / nthreads2w2
        );
      }
      ...
      var payoutEfficiency = Math.ceil(
        hpayout / (time2g + time2w + time2w2 + time2h)
      );
      ns.print(target);
      ns.print(
        hpayout +
  ...
  for (var i = 0; i < hostnames.length; i++) {
    if (
      ns.getServerMaxRam(hostnames[i]) > 0 &&
      ns.hasRootAccess(hostnames[i])
    ) {
  ...
  for (var i = 0; i < workableHostnames.length; i++) {
    ns.print(
      "Total Ram / New Ram / Server adding Ram : " +
        totalRam +
        " / " +
        ns.getServerMaxRam(workableHostnames[i]) +
        " / " +
        workableHostnames[i]
    );
  ...
  payoutSorted = payoutSorted.sort(function (a, b) {
    return b - a;
  });
  ...
  ns.print("spawning new instance");
